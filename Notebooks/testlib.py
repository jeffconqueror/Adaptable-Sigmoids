...
class knn_distinguish():
    ...
    def empirical_CDF(self, data, title):
        ...
        # s = sort_data.flatten()
        # print(len(s))
        # print(np.reshape(sort_data))
        #print("data len: ",len(sort_data))
        # if np.array(sort_data).ndim == 2:
        #     np.array(sort_data).ravel()
        # if sort_data.ndim != 1:
        #     sort_data.flatten()
        # x = np.concatenate(([0],s))
    ...
    def getPvalue(self, if_binning=False):
        ...
        sorted_sigmoid = sorted(self.store_MSE.items(), key=lambda x:x[1])
